        ...
        static class CaravanMaker_MakeCaravan_Patch
        {
            static void Postfix(ref Caravan __result) 
            {
                ...
                foreach (var pawn in __result.pawns)
                {
                    ...
                    caravanAutoFoodRestrictionsData.RetainedCaravanData[pawn.GetUniqueLoadID()] = pawn.foodRestriction.CurrentFoodRestriction.label;
        ...
        static class CaravanArrivalAction_Enter_Patch
        {
            static void Prefix(Caravan caravan, ref MapParent ___mapParent) 
            {
                ...
                if (map.IsPlayerHome)
                {
                    ...
                    {
                        if(caravanAutoFoodRestrictionsData.RetainedCaravanData.TryGetValue(pawn.GetUniqueLoadID(), out var pawnFoodRestrictionLabelSaved))
                        {
                            ...
                            caravanAutoFoodRestrictionsData.RetainedCaravanData.Remove(pawn.GetUniqueLoadID());
                        }
                        // else
                        // {
                        //     Log.Message("CaravanAutoFoodRestrictions Harmony CaravanArrivalAction_Enter value not found " + pawn.GetUniqueLoadID());
                        // }
                    }
                }
                // else
    ...
    public class CaravanAutoFoodRestrictionsMod : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect inRect)
        {
            ...
            if (Current.Game != null)
            {
                ...
                listingStandard.AddLabeledRadioList("Default Caravan Food Restriction", labelFoodRestrictions,ref Find.World.GetComponent<CaravanAutoFoodRestrictionsData>().PawnFoodRestrictionLabel);
            }
            else
            {
